...
public class DatabaseHelperTransmiSitp extends SQLiteOpenHelper {
    ...
    private static String DB_NAME = "transmi_sitp";
    private int appID;
    ...
    public DatabaseHelperTransmiSitp(Context context, int iAppID) {
    ...
    public String getListStringNearStation(Location myLocation, float distance, String stopTypeIds) {
        ...
        try {
            ...
            Log.d(DatabaseHelperTransmiSitp.class.toString(), "Error while trying to getListStringNearStation from database");
            if (!(cursor == null || cursor.isClosed())) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    public ArrayList<IntegerFloat> getListNearStation(Location myLocation, float distance, String stopTypeIds) {
        ...
        try {
            ...
            Log.d(DatabaseHelperTransmiSitp.class.toString(), "Error while trying to getListNearStation from database");
            if (!(cLocations == null || cLocations.isClosed())) {
                cLocations.close();
            }
        } catch (Throwable th) {
            ...
            if (cLocation.moveToFirst()) {
                ...
                if (!(fLat == 0.0f || fLon == 0.0f)) {
                    ...
                    try {
                        ...
                        location = firstLocation;
                        try {
                            Log.d(DatabaseHelperTransmiSitp.class.toString(), "Error while trying to calculaDistanciaUbicacion from database");
    ...
    public String getBusesWithOriginAndDestinationInMio(String listOrigin, String listDestination, String sWhere) {
        ...
        try {
            ...
            Log.d(DatabaseHelperTransmiSitp.class.toString(), "Error while trying to getBusesWithOriginAndDestinationIn from database");
            if (!(cursor == null || cursor.isClosed())) {
                cursor.close();
            }
        } catch (Throwable th) {
            ...
            if (cursor.moveToFirst()) {
                ...
                do {
                    try {
                        ...
                        station.setColor(cursor.getString(cursor.getColumnIndex("troncalColor")));
    ...
    public boolean getHorariosBuses(String idBus) {
        boolean result = false;
        String[] strArr = new String[DATABASE_VERSION_XAPASTO];
        strArr[0] = idBus;

        Cursor cursor = this.myDataBase.rawQuery("SELECT * FROM bus_horario WHERE pk_bus = ?", strArr);
        try {
            if (cursor.moveToFirst()) {
                result = true;
            } else {
    ...
    /*public void agregarConexionesMio(String _appID, ArrayList<Network> sWhere, AssetManager vertices, boolean asset, HashMap<String, Integer> calculoTroncal, HashMap hashReverseTotal) {
        String sql = "select DISTINCT b.rowid as _id, b.pk_id, b.fk_agency from bus b INNER JOIN bus_horario bh ON b.pk_id = bh.fk_bus WHERE " + sWhere + " ORDER BY _id";
        Cursor cursor = this.myDataBase.rawQuery(sql, null);
        int i = 0x1;
        try {
            if(cursor.moveToFirst()) {
                int iBus = cursor.getInt(cursor.getColumnIndex("pk_id"));
                String sAgency = cursor.getString(cursor.getColumnIndex("fk_agency"));
                byte[] bufferFLine = new byte[0x6];
                byte[] bufferLine = new byte[0x4];
                InputStream myInput = asset.open("sup" + iBus);
                myInput.read(bufferFLine);
                int iOrigenByte = ((bufferFLine[0x0] << 0x8) & 0xff00) | ((bufferFLine[0x1] << 0x0) & 0xff);
                int iDestinoByte = ((bufferFLine[0x2] << 0x8) & 0xff00) | ((bufferFLine[0x3] << 0x0) & 0xff);
                int iWByte = ((bufferFLine[0x4] << 0x8) & 0xff00) | ((bufferFLine[0x5] << 0x0) & 0xff);
                double dWByte;
                if((_appID == 1) && (sAgency.equals("TRANSMILENIO"))) {
                    double dWByte = 0.03 * (double)iWByte;
                } else {
                            dWByte = 0.03 * (double)iWByte;
                }
                HashMap<String, Integer> hashReverseBus = new HashMap<String, Integer>();
                int nodeOrigin = i;
                i = i + 0x1;
                int nodeDestination = i;
                hashReverseBus.put((sAgency + iOrigenByte + "_" + iBus, Integer.valueOf(nodeOrigin));
                hashReverseBus.put(((sAgency + iOrigenByte + "_" + iBus + iDestinoByte + "_" + iBus, Integer.valueOf(nodeDestination));
                Network temp = new Network(nodeOrigin, nodeDestination, dWByte);
                vertices.add(temp);
                while((int length = myInput.read(bufferLine)) > 0) {
                    nodeOrigin = nodeDestination;
                    if(length == 0x4) {
                        iDestinoByte = ((bufferLine[0x0] << 0x8) & 0xff00) | ((bufferLine[0x1] << 0x0) & 0xff);
                        iWByte = ((bufferLine[0x2] << 0x8) & 0xff00) | ((bufferLine[0x3] << 0x0) & 0xff);
                        if((_appID == 0x1) && (sAgency.equals("TRANSMILENIO"))) {
                            dWByte = 0.03 * (double)iWByte;
                        } else {
                            dWByte = 0.03 * (double)iWByte;
                        }
                        if(hashReverseBus.containsKey((sAgency + iDestinoByte + "_" + iBus)) {
                            nodeDestination = localObject1.intValue();
                            Network temp = new Network(nodeOrigin, nodeDestination, dWByte);
                            vertices.add(temp);
                            continue;
                        } else {
                            ...
                            Network temp = new Network(nodeOrigin, nodeDestination, dWByte);
                            vertices.add(temp);
                            hashReverseBus.put((vertices + iDestinoByte + "_" + iBus, Integer.valueOf(nodeDestination));
                        }
                    }
                    myInput.close();
                    hashReverseTotal.putAll(hashReverseBus);
                    i = i + 0x1;
                    if(!cursor.moveToNext()) {
                    }
                }
            } catch(Exception e) {
                Log.d("SITP", e.getMessage());
            } finally {
                if((cursor != null) && (!cursor.isClosed())) {
                    cursor.close();
                }
                sql = "select be.fk_estacion as stationId, b.pk_id as busId from bus b INNER JOIN bus_horario bh ON b.pk_id = bh.fk_bus INNER JOIN bus_estacion be ON b.pk_id = be.fk_bus WHERE " + sWhere + " AND be.show = 1 ORDER BY stationId";
                cursor = myDataBase.rawQuery(sql, 0x0);
                HashMap hashReverseBus = new HashMap();
                i = hashReverseTotal.size();
                int iPreviousStation = -0x1;
                try {
                    if(cursor.moveToFirst()) {
                        iBus = cursor.getInt(cursor.getColumnIndex("busId"));
                        int iStation = cursor.getInt(cursor.getColumnIndex("stationId"));
                        if(iStation != iPreviousStation) {
                            i = i + 0x1;
                            hashReverseBus.put((iStation + iStation + "_-1", Integer.valueOf(i));
                            iPreviousStation = iStation;
                        }
                        Network temp = new Network(i, localboolean2.intValue(), 0.0);
                        vertices.add(temp);
                        Network temp = new Network(localboolean2.intValue(), i, 0.0);
                        vertices.add(temp);
                        if(!cursor.moveToNext()) {
                            hashReverseTotal.putAll(hashReverseBus);
                        }
                    ...
                    if(calculoTroncal != null) {
                        String sql = "select fk_estacionOrigen, fk_estacionDestino, w from estacion_conexion WHERE fk_tipoConexion = 1";
                    } else {
                    }
                    cursor = myDataBase.rawQuery(sql, 0x0);
                    try {
                        if(cursor.moveToFirst()) {
                            int iOriginStation = cursor.getInt(cursor.getColumnIndex("fk_estacionOrigen"));
                            int iDestinationStation = cursor.getInt(cursor.getColumnIndex("fk_estacionDestino"));
                            double dW = cursor.getDouble(cursor.getColumnIndex("w"));
                            Integer iOriginHash = (Integer)hashReverseTotal.get((cursor.getColumnIndex("w") + iOriginStation + "_-1");
                            Integer iDestinationHash = (Integer)hashReverseTotal.get((hashReverseTotal + iDestinationStation + "_-1");
                            if((iOriginHash != null) && (iDestinationHash != null)) {
                                Network temp = new Network(iOriginHash.intValue(), iDestinationHash.intValue(), (0.15 * dW));
                                vertices.add(temp);
                                Network temp = new Network(iDestinationHash.intValue(), iOriginHash.intValue(), (0.15 * dW));
                                vertices.add(temp);
                            }
                            if(!cursor.moveToNext()) {
                            }
...
*/
    ...
    public List<BusView> armaListaBuses(Cursor cursor, String strFechaActual, boolean esFestivo, int type) {
        ...
        Log.v("aqui dentro","dentro de do"+result.toString());
    ...
    public List<BusView> getBusesFromStation(int id, String strFechaActual, boolean esFestivo) {
        List<BusView> result = null;
        Cursor cursor = this.myDataBase.rawQuery("select DISTINCT dias, * from (select  b.pk_id as id, b.fk_agency as agency, b.name as name, b.color as color, b.description as description, d.description as dias, bh.fk_dia as dia, bh.desde as desde, bh.hasta as hasta , be.vagon as vagon  from bus b INNER JOIN bus_horario bh ON b.pk_id = bh.fk_bus INNER JOIN dia d ON bh.fk_dia = d.pk_id INNER JOIN bus_estacion be ON b.pk_id = be.fk_bus INNER JOIN estacion e ON be.fk_estacion = e.pk_id where b.pk_id in ( select fk_bus from bus_estacion where fk_estacion = " + String.valueOf(id) + " and show=1) and be.fk_estacion=" + String.valueOf(id) + " order by name)  GROUP BY id,name", null);
        Log.v("sql---","select DISTINCT dias, * from (select  b.pk_id as id, b.fk_agency as agency, b.name as name, b.color as color, b.description as description, d.description as dias, bh.fk_dia as dia, bh.desde as desde, bh.hasta as hasta , be.vagon as vagon  from bus b INNER JOIN bus_horario bh ON b.pk_id = bh.fk_bus INNER JOIN dia d ON bh.fk_dia = d.pk_id INNER JOIN bus_estacion be ON b.pk_id = be.fk_bus INNER JOIN estacion e ON be.fk_estacion = e.pk_id where b.pk_id in ( select fk_bus from bus_estacion where fk_estacion = " + String.valueOf(id) + " and show=1) and be.fk_estacion=" + String.valueOf(id) + " order by name)  GROUP BY id,name");

        try {
            result = armarBusesDos(cursor, strFechaActual, esFestivo);
            if (!(cursor == null || cursor.isClosed())) {
                cursor.close();
            }
        } catch (Exception e) {
            Log.d(DatabaseHelperTransmiSitp.class.toString(), "Error while trying to getBusesFromStation from database--"+e.getMessage());
            if (!(cursor == null || cursor.isClosed())) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    public List<BusView> armarBusesDos(Cursor cursor, String strFechaActual,boolean esFestivo) throws Throwable {
        Throwable th;
        Time hora = new Time();
        hora.setToNow();
         List itemsHorario = null;
        ...
        try {
            if (cursor.moveToFirst()) {
                ...
                do {
                    ...
                    int id = cursor.getInt(cursor.getColumnIndex(ShareConstants.WEB_DIALOG_PARAM_ID));
                    String d = cursor.getString(cursor.getColumnIndex("dias"));
                    List<HorarioItem> itemsHorario2;
                    Integer iDiaId;
                    Time horaFrom;
                    Time horaTo;
                    int i;
                    ...
                    try {
                        ...
                        bv.setName(cursor.getString(cursor.getColumnIndex(MovilixaConstants.NAME)));
                        bv.setColor(cursor.getString(cursor.getColumnIndex("color")));
                        bv.setDescripcion(cursor.getString(cursor.getColumnIndex("description")));
                        ...
                        if (hora.after(horaFrom) && hora.before(horaTo)) {
                            if ((hora.weekDay == 0 || esFestivo) && iDiaId.intValue() == 4) {
                                ...
                                Log.v("iDialId---",iDiaId.intValue()+"");

                                 if (i == 6 && (iDiaId.intValue() == 2 || iDiaId.intValue() == 3)) {
                                    enOperacion = true;
                                } else if (hora.weekDay > 0) {
                                    i = hora.weekDay;
                                    if (i < 6 && (iDiaId.intValue() == DATABASE_VERSION_XAPASTO || iDiaId.intValue() == 2)) {
                                        enOperacion = true;
                                    }
                                }
                            }
                        ...
                        strDias = cursor.getString(cursor.getColumnIndex("dias"));
                        str = strDias;
                        str2 = horaFrom.format("%I:%M %p").replace(". ", XmlPullParser.NO_NAMESPACE).replace(".", XmlPullParser.NO_NAMESPACE) + " - " + horaTo.format("%I:%M %p").replace(". ", XmlPullParser.NO_NAMESPACE).replace(".", XmlPullParser.NO_NAMESPACE);
                        itemsHorario.add(new HorarioItem(str, str2, horaFrom.format("%I:%M %p").replace(". ", XmlPullParser.NO_NAMESPACE).replace(".", XmlPullParser.NO_NAMESPACE).replace("AM", "A M") + " A " + horaTo.format("%I:%M %p").replace(". ", XmlPullParser.NO_NAMESPACE).replace(".", XmlPullParser.NO_NAMESPACE).replace("AM", "A M")));
                        bv.setEnOperacion(enOperacion);
                        bv.setHorarios(itemsHorario);
                        if (type == DATABASE_VERSION_XAPASTO) {
                            bv.setVagon(XmlPullParser.NO_NAMESPACE);
    ...
    public List<Troncal> getTroncales() {
        ...
        try {
            ...
            Log.d(DatabaseHelperTransmiSitp.class.toString(), "Error while trying to getTroncales from database");
            if (!(cursor == null || cursor.isClosed())) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    public String getBusInfo(String idBus, String estacionOrigen, int iAppID) {
        ...
        if (!(idVag == null || idVag.compareTo(XmlPullParser.NO_NAMESPACE) == 0)) {
            sMsg = sMsg + "Ve hacia el vag\u00f3n " + idVag;
        }

        if (iAppID == 3) {
            sMsg = sMsg+"L\u00ednea " + sBus + " del " + sAgency + " con destino " + sDestino;
        } else if (iAppID == 4) {
            sMsg = sMsg+"Ruta " + sBus + " con destino " + sDestino;
        } else {
            sMsg =sMsg+" - Destino "+sDestino;
        }
    ...
    public ArrayList<RecorridoRuta> getRecorridosBusList(String query, int _appID, int iEstacionDestino, int iEstacionOrigen, double BRT_FACTOR, double BUS_FACTOR) {
        ...
        try {
            ...
            Log.d(DatabaseHelperTransmiSitp.class.toString(), "Error while trying to getRecorridosBusList from database");
            if (!(cursor == null || cursor.isClosed())) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    public String getIdBusFromName(String busName) {
        ...
        try {
            ...
            Log.d(DatabaseHelperTransmiSitp.class.toString(), "Error while trying to getIdBusFromName from database");
            if (!(cursor == null || cursor.isClosed())) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    public MyItem getItem(int idStation, int _resPin_mapTM, int _resPin_mapSITP) {
        MyItem myItem = null;
        String[] strArr = new String[DATABASE_VERSION_XAPASTO];
